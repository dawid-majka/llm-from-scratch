...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let run_dir = format!("trainig_output/{}", timestamp);
    ...
    // 256 - byte-level - no merges
    // only last one is close to real vocab sizes used by production grade models
    // GPT-2 and GPT-3 both use 50,257 tokens
    ...
    for &vocab_size in &vocab_sizes {
        ...
        println!("Encoding to measure compression...");
        ...
        test_roundtrip(&mut tokenizer)?;

        let save_path = format!("{}/tokenizer_{}.json", run_dir, vocab_size);
        tokenizer.save(&save_path)?;
        println!("\nSaved to: {}", save_path);

        // analyze vocab for non-byte-level tokenizers
        if vocab_size > 256 {
...
fn test_roundtrip(tokenizer: &mut Tokenizer) -> Result<(), Box<dyn std::error::Error>> {
